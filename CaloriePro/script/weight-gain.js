...
function submit() {
  ...
  if (kgPerWeek <= weight/100) {
    ...
    document.querySelector('.js-target')
      ...
      const HTML1 = `
        <p class="red">Your Surplus Calories</p>
        <p class="red">${surplus}</p>`

     document.querySelector('.js-surplus').innerHTML = HTML1;      
} else if (kgPerWeek > weight/100) {
     const HTML1 = `
        <p class="red">It's is not a preferrable weight gain</p>
        <p class="red">Please change (you want to weight gain) or (Number of weeks)</p>`
